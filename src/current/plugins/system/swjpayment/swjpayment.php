        ...
        switch (true) {
            case $this->_checkContext($context, self::_PROJECT_CONTEXT):
                ...
                if (!empty($key)) {
                    ...
                    $results = ECLTools::triggerEvent('onShowPaymentHTML', array(self::_PROJECT_CONTEXT, $item, $item_layout, $vars->order_number, &$paymentHTML));
    ...
    public function onAjaxSwjpayment()
    {
        ...
        switch ($action) {
            ...
            case 'request_payment_status':
                ...
                $results = ECLTools::triggerEvent('onGetPaymentInfo', array($order, &$response));
...
//                ));
                ...
                switch ($response['payment_status'] ?? SWJPaymentStatuses::SWJPAYMENT_STATUS_CANCELED) {
                    ...
                    case SWJPaymentStatuses::SWJPAYMENT_STATUS_PENDING:
                        ...
                        // Отправить чек
                        if ($response['payment_status'] == SWJPaymentStatuses::SWJPAYMENT_STATUS_CONFIRMED) {
                            ECLTools::triggerEvent('onReadyToSendReceipt', array($key_id));
                        }
                        break;
            ...
            case 'payment-notification':
                ...
                $results = ECLTools::triggerEvent('onProcessPayment', array($processor, $post, &$response));
                ...
                switch ($response['payment_status'] ?? SWJPaymentStatuses::SWJPAYMENT_STATUS_CANCELED) {
                    ...
                    case SWJPaymentStatuses::SWJPAYMENT_STATUS_PENDING:
                        ...
                        // Отправить чек
                        if ($response['payment_status'] == SWJPaymentStatuses::SWJPAYMENT_STATUS_CONFIRMED) {
                            ECLTools::triggerEvent('onReadyToSendReceipt', array($order->id));
                        }
                        break;
                    case SWJPaymentStatuses::SWJPAYMENT_STATUS_REFUND:
                    case SWJPaymentStatuses::SWJPAYMENT_STATUS_DENIED:
            ...
            case 'create_order':
                ...
                $data["amount"] = $input->get('project_price', 0);
