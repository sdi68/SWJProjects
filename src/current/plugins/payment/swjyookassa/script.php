...
if (!class_exists('plgPaymentSWJYookassaInstallerScript'))
{
	/**
	 * Скрипт установки платежного плагина YOOKassa
	 * @package     pkg_swjprojects_payments
	 * @subpackage  payment/swjyookassa
	 * @version     1.0.0
	 * @since       1.0.0
	 */
	class plgPaymentSWJYookassaInstallerScript
	{
		public function preflight($type, $parent)
		{
			...
			{
				$app->enqueueMessage('The plugin requires an installed and activated plugin swjpayment to work! Install please this before!', 'error');
		...
		public function postflight($type, $parent)
		{
			...
			$query->select('extension_id')
				->from($db->qn('#__extensions'))
				->where($db->qn('type') . ' = ' . $db->q('plugin'))
				->where($db->qn('folder') . ' = ' . $db->q('payment'))
				->where($db->qn('element') . ' = ' . $db->q('swjyookassa'));
            ...
            <h3>the SWJProjects Payment Gateway YOOKASSA Plugin v1.0.0 Changelog</h3>
            ...
            <a class="btn btn-primary btn-large"
               href="<?php echo Route::_('index.php?option=com_plugins&task=plugin.edit&extension_id=' . $pluginId); ?>">Start
                using the SWJProjects Payment Gateway YOOKASSA Plugin.</a>
		<?php } ?>
			<?php if (0): ?>
